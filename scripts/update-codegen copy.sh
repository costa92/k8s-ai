...
PROJ_MODULE_NAME="github.com/costa92/k8s-api"
#PLURAL_EXCEPTIONS=""
...
OUTPUT_PKG="github.com/costa92/k8s-api/pkg/generated"
...
if [[ "${DBG_CODEGEN}" == 1 ]]; then
    proj::log::status "DBG: starting generated_files"
fi

# Generate a list of directories we don't want to play in.
DIRS_TO_AVOID=()
proj::util::read-array DIRS_TO_AVOID < <(
...
if [[ "${DBG_CODEGEN}" == 1 ]]; then
    proj::log::status "DBG: finding all +k8s: tags"
fi
ALL_K8S_TAG_FILES=()
proj::util::read-array ALL_K8S_TAG_FILES < <(
    git_grep -l \
...
if [[ "${DBG_CODEGEN}" == 1 ]]; then
    proj::log::status "DBG: found ${#ALL_K8S_TAG_FILES[@]} +k8s: tagged files"
fi
...
function codegen::protobuf() {
    ...
    # source tree.  This is managed in proj::build::copy_output in build/common.sh.
    # If the output set is changed update that function.

    local apis=()
    proj::util::read-array apis < <(
        git grep --untracked --null -l \
            -e '// +k8s:protobuf-gen=package' \
            -- \
            cmd pkg \
            | while read -r -d $'\0' F; do dirname "${F}"; done \
            | sed 's|^|github.com/costa92/k8s-api/|;s|k8s.io/kubernetes/staging/src/||' \
            | sort -u)

    proj::log::status "Generating protobufs for ${#apis[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: generating protobufs for:"
        for dir in "${apis[@]}"; do
            proj::log::status "DBG:     $dir"
        done
    fi
    ...
    # proj-apiserver: undefined: ObjectReference.
    ...
    if proj::protoc::check_protoc >/dev/null; then
      scripts/_update-generated-protobuf-dockerized.sh "${apis[@]}"
    else
      proj::log::status "protoc ${PROTOC_VERSION} not found (can install with scripts/install-protoc.sh); generating containerized..."
...
function codegen::deepcopy() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: finding all +k8s:deepcopy-gen tags"
    fi
    local tag_dirs=()
    proj::util::read-array tag_dirs < <( \
        grep -l --null '+k8s:deepcopy-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            | while read -r -d $'\0' F; do dirname "${F}"; done \
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: found ${#tag_dirs[@]} +k8s:deepcopy-gen tagged dirs"
    fi
    ...
    proj::log::status "Generating deepcopy code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: running deepcopy-gen for:"
        for dir in "${tag_dirs[@]}"; do
            proj::log::status "DBG:     $dir"
        done
    fi
    ...
    deepcopy-gen \
        ...
        --bounding-dirs "${PROJ_MODULE_NAME},k8s.io/api" \
        "${tag_pkgs[@]}" \
        "$@"

    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "Generated deepcopy code"
    fi
}
...
function gen_types_swagger_doc() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: running gen-swaggertype-docs for ${group_version} at ${gv_dir}"
    fi
...
function codegen::swagger() {
    ...
    proj::log::status "Generating swagger for ${#group_versions[@]} targets"

    git_find -z ':(glob)**/types_swagger_doc_generated.go' | xargs -0 rm -f

    # Regenerate files.
    for group_version in "${group_versions[@]}"; do
      gen_types_swagger_doc "${group_version}" "$(proj::util::group-version-to-pkg-path "${group_version}")"
    done
}
...
function codegen::prerelease() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: finding all +k8s:prerelease-lifecycle-gen tags"
    fi
    local tag_dirs=()
    proj::util::read-array tag_dirs < <( \
        grep -l --null '+k8s:prerelease-lifecycle-gen=true' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: found ${#tag_dirs[@]} +k8s:prerelease-lifecycle-gen tagged dirs"
    fi
    ...
    proj::log::status "Generating prerelease-lifecycle code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: running prerelease-lifecycle-gen for:"
        for dir in "${tag_dirs[@]}"; do
            proj::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "Generated prerelease-lifecycle code"
    fi
}
...
function codegen::defaults() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: finding all +k8s:defaulter-gen tags"
    fi
    local tag_dirs=()
    proj::util::read-array tag_dirs < <( \
        grep -l --null '+k8s:defaulter-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: found ${#tag_dirs[@]} +k8s:defaulter-gen tagged dirs"
    fi
    ...
    proj::log::status "Generating defaulter code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: running defaulter-gen for:"
        for dir in "${tag_dirs[@]}"; do
            proj::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "Generated defaulter code"
    fi
}
...
function codegen::conversions() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: finding all +k8s:conversion-gen tags"
    fi
    local tag_dirs=()
    proj::util::read-array tag_dirs < <(\
        grep -l --null '^// *+k8s:conversion-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: found ${#tag_dirs[@]} +k8s:conversion-gen tagged dirs"
    fi
    ...
    proj::log::status "Generating conversion code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: running conversion-gen for:"
        for dir in "${tag_dirs[@]}"; do
            proj::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "Generated conversion code"
    fi
}
...
function todo::codegen::openapi() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: finding all +k8s:openapi-gen tags"
    fi

    local tag_files=()
    proj::util::read-array tag_files < <(
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: found ${#tag_dirs[@]} +k8s:openapi-gen tagged dirs"
    fi
    ...
    proj::log::status "Generating openapi code"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "DBG: running openapi-gen for:"
        for dir in "${tag_dirs[@]}"; do
            proj::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        proj::log::status "Generated openapi code"
    fi
}
...
function codegen::openapi() {
    ...
    local output_dir="pkg/generated/openapi"
    local output_pkg="github.com/costa92/k8s-api/${output_dir}"
    local known_violations_file="${API_KNOWN_VIOLATIONS_DIR}/violation_exceptions.list"
...
function codegen::listers() {
    ...
    onex::util::read-array ext_apis < <(
        cd "${PROJ_ROOT_DIR}"
        git_find -z ':(glob)pkg/apis/**/*types.go' \
            | while read -r -d $'\0' F; do dirname "github.com/costa92/k8s-api/${F}"; done \
            | sort -u)
    ext_apis+=("${EXTRA_GENERATE_PKG[@]}")
