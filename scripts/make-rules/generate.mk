...
gen.systemd.%: ## Generate specified systemd unit file.
	$(eval PROJ_ENV_FILE ?= $(MANIFESTS_DIR)/env.local)
	$(eval GENERATED_SERVICE_DIR := $(OUTPUT_DIR)/systemd)
	$(eval SERVICE := $(lastword $(subst ., ,$*)))
	@echo "===========> Generating $(SERVICE) systemd unit file"
	@$(SCRIPTS_DIR)/gen-service-config.sh $(SERVICE) $(PROJ_ENV_FILE) \
		$(PROJ_ROOT_DIR)/configs/proj.systemd.tmpl.service $(GENERATED_SERVICE_DIR)
ifeq ($(V),1)
